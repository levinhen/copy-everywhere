...
- When a phase-sized chunk of work is complete and basic verification has passed, default to finishing the cycle with `commit & push` without waiting for an extra approval round.
- Only pause before pushing if there is a real blocker or risk, such as failed verification, unresolved conflicts, or uncertainty about whether local-only/untracked files should be included.
...
- **LAN HTTP requires cleartext opt-in.** The Android app talks to self-hosted LAN servers over `http://` IP addresses (for example `http://192.168.x.x`). Keep `android:usesCleartextTraffic="true"` on the application in `AndroidManifest.xml`, otherwise Android 9+ blocks the connection with "cleartext communication not permitted by network security policy".
