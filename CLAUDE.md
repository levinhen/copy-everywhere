...
- **Go server auto-build (Windows).** `CopyEverywhere.csproj` has a `BuildGoServer` MSBuild target (`BeforeTargets="Build"`, incremental via `Inputs`/`Outputs` over `server/**/*.go` + `go.mod`/`go.sum`) that runs `go build -o $(OutDir)copyeverywhere-server.exe .` from `server/` so `dotnet build`/`dotnet run` produces a co-located server binary. If `go version` fails the target emits a Warning (not an error) and skips — the WPF app already shows a "Server binary not found" UI message in that case. Don't `<Content Include>` the binary; let the target write directly into `$(OutDir)`.
...
- **MainWindow gear-toggle layout (Windows).** All configuration UI (server config, target device, transfer mode, embedded server, Bluetooth pairing, floating ball toggle) lives inside `<StackPanel x:Name="ConfigSectionsPanel">` and is hidden by default. The header has a gear button (Segoe MDL2 Assets `\uE713` / `\uE72B` for back) wired to `GearButton_Click` → `UpdateViewMode()` which swaps `ConfigSectionsPanel` ↔ `ClipboardPanel` + `MainPanelPlaceholder`. `_inSettingsView` defaults to true on first launch when `!IsConfigured && TransferMode == LanServer`. `UpdateMainPanelState()` is split into two halves: background services (SSE, queue polling, device list) start/stop based on config regardless of view mode, while the visibility toggle is gated by `if (_inSettingsView) return;` so flipping into settings doesn't tear down active connections. New config UI must go inside `ConfigSectionsPanel`, never as a peer.
