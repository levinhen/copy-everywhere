...
- **[macos/CopyEverywhere/](macos/CopyEverywhere/)** — Swift Package Manager executable target (no .xcodeproj), macOS 13+ MenuBarExtra app. State lives in `ConfigStore` (`@MainActor` ObservableObject) and `HistoryStore`; views in `MenuBarView` / `MainPanelView` / `ConfigView`. Optionally manages an embedded Go server subprocess via `ServerProcess` / `ServerConfig` (see Embedded Server section below).
...
- **mDNS service broadcast.** `discovery/` package wraps `github.com/hashicorp/mdns` (pure-Go, no CGO). Service type is `_copyeverywhere._tcp`. TXT records carry `version`, `auth`, and `server_id` fields; `server_id` must match the `/health` payload so clients can survive DHCP address changes. `main.go` starts mDNS after config load and deregisters on SIGINT/SIGTERM.
- **LAN discovery restore contract is shared across clients.** Keep `tasks/lan-discovery-selection-contract.md` aligned with the active Ralph files in `scripts/ralph/`. Client code should treat `server_id` as the durable selection key, `host:port` as display/transport metadata only, and use the shared source vocabulary `auto_discovered` / `restored_selection` / `manual_fallback`.
- **LAN discovery diagnostics should split logs between discovery and selection.** Emit discovery-start and resolved-`server_id` logs from the mDNS/Bonjour layer, then log auto-select / restored-selection / multi-server-wait / manual-fallback outcomes at the restore decision point so DHCP regressions can be reconstructed from one log stream.
- **Manual fallback URL must be persisted separately from the active discovered endpoint.** When a client selects a discovered LAN server, keep the last user-entered manual URL in its own persisted field so a later discovery miss can restore that fallback instead of leaving a stale discovered `host:port` active.
- **Stable server identity lives in `STORAGE_PATH/server_id`.** Use `config.LoadOrCreateServerID()` to create/read the persisted ID before wiring routes; `/health` must expose the same `server_id` value so DHCP-sensitive clients can verify server identity across restarts.
...
- **Persisted LAN selection must stay `server_id`-centric.** Use `StoredLanServerSelection.serverID` as the durable identity for discovered servers, keep `host:port` as last-known display/transport metadata only, and clear the stored selection back to `manual_fallback` when the user edits the Host URL away from the selected discovered endpoint.
- **macOS LAN restore logic belongs in `ConfigStore`, not Config view lifecycle.** Start Bonjour browsing when LAN mode becomes active, observe `bonjourBrowser.$discoveredServers`, and drive unique-server auto-select / persisted-`server_id` restore there so launch-time DHCP recovery works even before the Config popover is opened.
- **macOS LAN Config UI should render from selection state, not URL guessing.** Drive status badges, passive multi-server guidance, and manual/discovered switching from `lanEndpointSource`, `selectedLanServer`, and `bonjourBrowser.discoveredServers`; `hostURL` alone is not enough to explain whether the app is auto-discovered, restored, or manual fallback.
- **macOS targeted raw receives must include `device_id`.** Manual queue receive and SSE auto-receive should share a helper that builds `/api/v1/clips/:id/raw?device_id=<self>` so targeted fallback recovery uses the authoritative delivery-acknowledgement path.
...
**Embedded Server (macOS — `macos/CopyEverywhere/`):**

- **Embedded server architecture.** The standalone `CopyEverywhereServer` host app has been removed. Its `ServerProcess` and `ServerConfig` classes are now embedded directly in the CopyEverywhere client app. The server is an optional feature toggled via `serverEnabled` in `ServerConfig`.
...
- **ServerConfig persistence.** `ServerConfig` (`@MainActor` ObservableObject) persists port, storage path, TTL, auth settings to `~/Library/Application Support/CopyEverywhere/server-config.json`. Default storage path is `~/Library/Application Support/CopyEverywhere/server-data`. Config changes require a server restart to take effect.
- **Server toggle and auto-connect.** `serverEnabled` toggles the subprocess on/off and auto-sets `hostURL` to `http://localhost:<port>`. `autoStartServer` starts the server on app launch. Both are persisted in `ServerConfig`.
- **Server UI.** `ConfigView` has an embedded server section (port, bind address, storage path, TTL, max clip size, auth toggle, auto-start). `MainPanelView` shows server status (green/red/gray dot), Start/Stop/Restart buttons, log viewer, storage stats, and connected devices list.
...
- **`restart()` polls for process exit** (up to 5s) instead of a fixed delay — prevents duplicate mDNS registrations when the old process is slow to exit.
- **Android does not embed the server** — it connects to a server hosted by macOS or Windows.
...
- **Windows LAN selection persistence must stay `server_id`-centric.** Persist discovered selections through `ConfigStore.SelectedLanServer.ServerId`, keep `HostUrl` as manual/transport metadata, and normalize the manual Host URL before `Save()` so typing over a discovered endpoint clears stale selection state back to `manual_fallback` instead of keying selection off `host:port`.
...
- **mDNS discovery.** `MdnsDiscoveryService` (`data/MdnsDiscoveryService.kt`) wraps `NsdManager` to browse for `_copyeverywhere._tcp.` services. `DiscoveredServer` data class holds `serverId`, name, host, port, authRequired, version. TXT record attributes parsed from `NsdServiceInfo.attributes`. Discovery is owned by `CopyEverywhereService` in LAN mode, while the Config UI only observes the shared `discoveredLanServers` state. No extra permissions needed beyond `INTERNET`.
...
- **Android LAN selection persistence should flow through `ConfigStore` helpers.** Use `selectDiscoveredServer(...)`, `updateManualHostUrl(...)`, and `useManualLanFallback()` so `SelectedLanServer.serverId` remains the durable key, `LanEndpointSource` stays on the shared snake_case contract, and manual URL edits clear stale discovered selection state instead of preserving host-only matches.
- **Android LAN startup discovery belongs in `CopyEverywhereService`, not Config UI lifecycle.** Start `MdnsDiscoveryService` when the foreground service enters LAN mode, expose the live discovered-server list through `CopyEverywhereService.discoveredLanServers`, restore by `SelectedLanServer.serverId`, and only auto-pick a single discovered server when there is no saved selection and no manual Host URL fallback.
- **Android LAN config UI should render from selection state, not Host URL guessing.** Drive source copy, manual-fallback actions, passive multi-server guidance, and discovered-server selection badges from `lanEndpointSource`, `selectedLanServer`, and the shared discovery list; `hostUrl` is only the current endpoint text, not the authoritative source state.
...
- **Windows LAN startup restore belongs in `MainWindow`, not just config persistence.** Keep mDNS browsing running from launch, but apply auto-select/restore only when `TransferMode == LanServer`: restore by `SelectedLanServer.ServerId`, auto-pick only when there is exactly one discovered server and no saved/manual URL, and persist the updated `HostUrl` immediately after discovery rewrites it.
- **Windows LAN UI should expose source state directly from `ConfigStore`.** Render discovery badges/guidance from `LanEndpointSource`, `SelectedLanServer`, and the current Bonjour result count, and use an explicit manual-fallback action instead of relying on the user to mutate `HostUrl` just to clear a discovered selection.
