...
// ListTargetedPendingClips returns unconsumed, ready, non-expired clips that
// explicitly target the given device. Results are newest first.
func (d *DB) ListTargetedPendingClips(deviceID string) ([]*Clip, error) {
	rows, err := d.conn.Query(`
